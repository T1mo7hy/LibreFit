    ...
    LibreFitLazyColumn(lazyListState = lazyListState) {
        ...
        if (workoutItems.isEmpty()) {
            ...
            itemsIndexed(
                ...
                ReorderableItem(
                    reorderableLazyListState, key = when (workoutItem) {
                        is UiWarmupItem -> "w"; is UiExerciseItem -> "e"
                    } + workoutItem.id.toString()
                ) { isDragging ->
                    when (workoutItem) {
    ...
    val e: List<UiWorkoutItem> = listOf(
        ...
        UiWarmupItem(
            UiWarmupWithSets(
                ...
                sets = persistentListOf(
                    UiSet(load = Weight.kilograms(80.0), reps = 10, completed = true),
                    UiSet(load = Weight.kilograms(80.0), reps = 6, completed = true),
                    UiSet(load = Weight.kilograms(80.0), reps = 3, completed = true),
                )
            )
        ),
        UiExerciseItem(
